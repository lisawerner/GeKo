...
int main()
{
	...
	VertexShader vsVelocity(loadShaderSource(SHADERS_PATH + std::string("/Velocity/Velocity.vert")));
	FragmentShader fsVelocity(loadShaderSource(SHADERS_PATH + std::string("/Velocity/Velocity.frag")));
	ShaderProgram shaderVelocity(vsVelocity, fsVelocity);
	...
	FBO fboVelocity(WINDOW_WIDTH, WINDOW_HEIGHT, 3, true, false);
	...
	while (!glfwWindowShouldClose(testWindow.getWindow()))
	{
		...
		shaderBlur.sendMat4("uPrevModelViewProjectionMat", prev);
		screenFillingQuad.renderGeometry();
		//testScene.render(shaderBlur);
		shaderVelocity.unbind();
		fboVelocity.unbind();
		...
		fboResult.bind();
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		shaderBlur.sendSampler2D("colorTexture", fboGBuffer.getColorTexture(2), 2);
		shaderBlur.sendSampler2D("velocityTexture", fboVelocity.getColorTexture(2), 2);
		screenFillingQuad.renderGeometry();
		//testScene.render(shaderBlur);
		shaderBlur.unbind();
		fboResult.unbind();

		prev = cam.getProjectionMatrix() * cam.getViewMatrix() * (testScene.getScenegraph()->getRootNode()->getModelMatrix());
				
		//ScreenFillingQuad Render Pass
		shaderSFQ.bind();
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		shaderSFQ.sendSampler2D("fboTexture", fboVelocity.getColorTexture(2), 2);
		screenFillingQuad.renderGeometry();
		shaderSFQ.unbind();
