...
int main()
{
	...
	ShaderProgram depthShader(vsDepth, fsDepth);
	...
	ShaderProgram shaderDoF(vsDoF, fsDoF);
	...
	FBO depth_fbo(WINDOW_WIDTH, WINDOW_HEIGHT, 1, true, false);
	...
	while (!glfwWindowShouldClose(testWindow.getWindow()))
	{
		...
		depthShader.bind();
		depthShader.sendMat4("viewMatrix", cam.getViewMatrix());
		depthShader.sendMat4("projectionMatrix", cam.getProjectionMatrix());

		//Render the scene
		testScene.render(depthShader);
		...
		shaderDoF.sendSampler2D("tex", fboGBuffer.getColorTexture(2), 2);
		shaderDoF.sendSampler2D("depth", depth_fbo.getDepthTexture(), 1);

		float focus = (2.0f / cam.getFar());
		
		shaderDoF.sendFloat("focus_depth", focus);
