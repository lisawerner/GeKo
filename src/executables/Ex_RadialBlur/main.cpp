...
int main()
{
	...
	FBO fboRadialBlur(WINDOW_WIDTH, WINDOW_HEIGHT, 1, false, false);
	...
	while (!glfwWindowShouldClose(testWindow.getWindow()))
	{
		...
		fboRadialBlur.bind();
		shaderBlur.sendSampler2D("tex", fboGBuffer.getColorTexture(2));
		screenFillingQuad.renderGeometry();
		//testScene.render(shaderBlur);
		fboRadialBlur.unbind();
		...
		shaderSFQ.bind();
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		shaderSFQ.sendSampler2D("fboTexture", fboRadialBlur.getColorTexture(0));
		screenFillingQuad.renderGeometry();
		shaderSFQ.unbind();
